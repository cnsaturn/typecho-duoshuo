...
class Duoshuo_Client{
	...
	public function __construct($shortName = null, $secret = null, $jwt = null){
        ...
        $service = Duoshuo_Typecho::getInstance();
        $this->userAgent = $this->generator . '|Duoshuo/'. $service::VERSION;
        if($service->getPlugOption('end_point') !== NULL)
            $this->end_point = $service->PointMap[intval($service->getPlugOption('end_point'))];
	}
